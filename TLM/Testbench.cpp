...
Testbench::Testbench(sc_module_name n) : sc_module(n), initiator("initiator"), output_rgb_raw_data_offset(54)
{
  SC_THREAD(feed_rgb);
...
void Testbench::feed_rgb()
{
  ...
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width; j++)
    {
      ...
      mask[2] = 0xff;
      mask[3] = 0;
      initiator.write_to_socket(DC_MM_BASE + DC_FILTER_R_ADDR, mask, data.uc, 4);
      total_pixel = total_pixel + 1;
    }

  std::cout << std::endl;
  std::cout << "The number of pixels transfer from Testbench to DC_Filter = " << total_pixel << std::endl;
  sc_stop();
}
...
void Testbench::fetch_result()
{
  ...
  for (int i = 0; i < height; i++)
  {
    for (int j = 0; j < width; j++)
    {
      initiator.read_from_socket(DC_MM_BASE + DC_FILTER_RESULT_ADDR, mask, data.uc, 4);
