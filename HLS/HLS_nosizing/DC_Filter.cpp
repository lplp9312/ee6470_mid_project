...
const int mask1[MASK_X][MASK_Y] = {{1, 2, 1},
										  {2, 4, 2},
										  {1, 2, 1}};
const int mask2[MASK_X][MASK_Y] = {{1, 2, 1},
										  {2, 4, 2},
										  {1, 2, 1}};
const int factor1 = 16;
const int factor2 = 16;
...
void DC_Filter::do_filter_1()
{
	...
	int xBound = MASK_X / 2;
	int yBound = MASK_Y / 2;
	int WIDTH_yBound = WIDTH + yBound;
	int HEIGHT_xBound = HEIGHT + xBound;
	int HEIGHT_2xBound = HEIGHT + 2 * xBound;
		...
		for (k = 0; k != WIDTH; k++)
		{
			...
			for (h = 0; h != MASK_Y; h++)
				for (i = 0; i != MASK_X; i++)
					for (j = 0; j != MASK_N; j++)
					{
						f1_val[j] += f1_array_rgb[j][i][h + k] * mask1[i][h] / factor1;
					}

			rgb.range(7, 0) = f1_val[0];
			rgb.range(15, 8) = f1_val[1];
			rgb.range(23, 16) = f1_val[2];
		...
		for (k = 0; k != WIDTH; k++)
		{
			...
			for (h = 0; h != MASK_Y; h++)
				for (i = 0; i != MASK_X; i++)
					for (j = 0; j != MASK_N; j++)
					{
						f2_val[j] += f2_array_rgb[j][i][h + k] * mask2[i][h] / factor2;
					}

			rgb.range(7, 0) = f2_val[0];
			rgb.range(15, 8) = f2_val[1];
			rgb.range(23, 16) = f2_val[2];
