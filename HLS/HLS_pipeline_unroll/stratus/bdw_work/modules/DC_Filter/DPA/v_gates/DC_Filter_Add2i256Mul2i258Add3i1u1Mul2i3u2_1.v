`timescale 1ps / 1ps

// Generated by Cadence Genus(TM) Synthesis Solution 17.11-s014_1
// Generated on: May  5 2021 00:45:22 CST (May  4 2021 16:45:22 UTC)

module DC_Filter_Add2i256Mul2i258Add3i1u1Mul2i3u2_1(in2, in1, out1);
  input [1:0] in2;
  input in1;
  output [11:0] out1;
  wire [1:0] in2;
  wire in1;
  wire [11:0] out1;
  wire csa_tree_add_29_2_groupi_n_0, csa_tree_add_29_2_groupi_n_1,
       csa_tree_add_29_2_groupi_n_4, csa_tree_add_29_2_groupi_n_13,
       csa_tree_add_29_2_groupi_n_14, csa_tree_add_35_2_groupi_n_1,
       csa_tree_add_35_2_groupi_n_2, csa_tree_add_35_2_groupi_n_4;
  assign out1[0] = 1'b0;
  assign out1[5] = 1'b0;
  assign out1[6] = 1'b0;
  assign out1[7] = 1'b0;
  INVX1 g10(.A (out1[1]), .Y (out1[8]));
  ADDHX1 csa_tree_add_29_2_groupi_g82(.A (in2[1]), .B
       (csa_tree_add_29_2_groupi_n_4), .CO (out1[4]), .S (out1[3]));
  ADDFX1 csa_tree_add_29_2_groupi_g83(.A
       (csa_tree_add_29_2_groupi_n_14), .B (in2[0]), .CI (in2[1]), .CO
       (csa_tree_add_29_2_groupi_n_4), .S (out1[2]));
  NAND2X1 csa_tree_add_29_2_groupi_g85(.A
       (csa_tree_add_29_2_groupi_n_1), .B
       (csa_tree_add_29_2_groupi_n_13), .Y (out1[1]));
  NAND2X1 csa_tree_add_29_2_groupi_g86(.A (in2[0]), .B (in1), .Y
       (csa_tree_add_29_2_groupi_n_1));
  NOR2X8 csa_tree_add_29_2_groupi_g88(.A (in2[0]), .B (in1), .Y
       (csa_tree_add_29_2_groupi_n_0));
  INVXL csa_tree_add_29_2_groupi_fopt(.A
       (csa_tree_add_29_2_groupi_n_0), .Y
       (csa_tree_add_29_2_groupi_n_13));
  CLKINVX3 csa_tree_add_29_2_groupi_fopt89(.A
       (csa_tree_add_29_2_groupi_n_0), .Y
       (csa_tree_add_29_2_groupi_n_14));
  MXI2X1 csa_tree_add_35_2_groupi_g166(.A
       (csa_tree_add_35_2_groupi_n_1), .B (out1[4]), .S0
       (csa_tree_add_35_2_groupi_n_4), .Y (out1[11]));
  ADDHX1 csa_tree_add_35_2_groupi_g167(.A (out1[3]), .B
       (csa_tree_add_35_2_groupi_n_2), .CO
       (csa_tree_add_35_2_groupi_n_4), .S (out1[10]));
  ADDHXL csa_tree_add_35_2_groupi_g168(.A (out1[2]), .B (out1[1]), .CO
       (csa_tree_add_35_2_groupi_n_2), .S (out1[9]));
  INVXL csa_tree_add_35_2_groupi_g169(.A (out1[4]), .Y
       (csa_tree_add_35_2_groupi_n_1));
endmodule

